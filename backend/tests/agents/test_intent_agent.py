...
Tests the IntentAgent (with mocked BedrockService) and IntentParser
...
@pytest.fixture
def mock_bedrock() -> AsyncMock:
    """Create a mocked BedrockService."""
    service = AsyncMock()
    return service


@pytest.fixture
def intent_agent(mock_bedrock: AsyncMock) -> IntentAgent:
    """Create an IntentAgent with mocked Bedrock."""
    return IntentAgent(bedrock_service=mock_bedrock)
...
class TestIntentExtraction:
    ...
    async def test_extracts_grocery_intent(
        self, intent_agent: IntentAgent, mock_bedrock: AsyncMock
    ) -> None:
        """Extracts a groceries category from user input."""
        mock_bedrock.invoke.return_value = _make_response(
    ...
    @pytest.mark.asyncio
    async def test_passes_user_input_to_bedrock(
        self, intent_agent: IntentAgent, mock_bedrock: AsyncMock
    ) -> None:
        """User input is forwarded to Bedrock as user_prompt."""
        mock_bedrock.invoke.return_value = _make_response()
...
class TestConfidenceScore:
    ...
    async def test_high_confidence_score(
        ...
        mock_bedrock.invoke.return_value = _make_response(confidence=0.97)
    ...
    async def test_low_confidence_score(
        ...
        mock_bedrock.invoke.return_value = _make_response(confidence=0.35)
    ...
    async def test_confidence_at_boundary_zero(
        ...
        mock_bedrock.invoke.return_value = _make_response(confidence=0.0)
    ...
    async def test_confidence_at_boundary_one(
        ...
        mock_bedrock.invoke.return_value = _make_response(confidence=1.0)
...
class TestInvalidInput:
    ...
    def test_parser_handles_markdown_wrapped_json(self) -> None:
        ...
        raw = '```json\n{"category": "baby", "urgency": "high", "confidence": 0.9}\n```'
    ...
    async def test_agent_propagates_parsing_exception(
        ...
        mock_bedrock.invoke.return_value = "Totally garbled output from LLM"
        with pytest.raises(IntentParsingException):
    ...
    async def test_agent_propagates_validation_exception(
        ...
        mock_bedrock.invoke.return_value = json.dumps({"category": "food"})
        with pytest.raises(IntentValidationException):
            await intent_agent.analyze("Some valid user input")
...
class TestEmptyQuery:
    ...
    @pytest.mark.asyncio
    async def test_empty_string_passed_to_bedrock(
        self, intent_agent: IntentAgent, mock_bedrock: AsyncMock
    ) -> None:
        """Empty string is still passed to Bedrock (validation is at API layer)."""
        mock_bedrock.invoke.return_value = _make_response(
    ...
    @pytest.mark.asyncio
    async def test_whitespace_only_passed_to_bedrock(
        self, intent_agent: IntentAgent, mock_bedrock: AsyncMock
    ) -> None:
        """Whitespace-only input is passed to Bedrock."""
        mock_bedrock.invoke.return_value = _make_response(
    ...
    @pytest.mark.asyncio
    async def test_bedrock_called_even_for_empty_input(
        self, intent_agent: IntentAgent, mock_bedrock: AsyncMock
    ) -> None:
        """Bedrock is still invoked for empty input (agent doesn't pre-filter)."""
        mock_bedrock.invoke.return_value = _make_response()
...
class TestAgentResponseStructure:
    ...
    def test_response_has_required_fields(self) -> None:
        """IntentResponse requires category, urgency, confidence."""
        response = IntentResponse(
            category="groceries", urgency="medium", confidence=0.8
        )
    ...
    def test_response_optional_fields_default_to_none(self) -> None:
        ...
        response = IntentResponse(
            category="food", urgency="low", confidence=0.5
        )
    ...
    def test_response_json_serialization(self) -> None:
        ...
        response = IntentResponse(
            category="baby", urgency="high", confidence=0.88
        )
    ...
    def test_response_urgency_literal_values(self) -> None:
        ...
        for urgency in ["low", "medium", "high", "critical"]:
            response = IntentResponse(
                category="other", urgency=urgency, confidence=0.5
            )
    ...
    def test_response_rejects_invalid_urgency(self) -> None:
        ...
        with pytest.raises(Exception):
            IntentResponse(category="other", urgency="extreme", confidence=0.5)
    ...
    async def test_full_pipeline_response(
        ...
        """End-to-end: input → Bedrock mock → parsed IntentResponse."""
        mock_bedrock.invoke.return_value = json.dumps({
            "category": "baby",
            "urgency": "critical",
            "budget": 30.0,
            "people_count": 1,
